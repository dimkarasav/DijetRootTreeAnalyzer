...
usage ()
{
	...
	echo ""
	echo "Usage:   $0 -list_No 'Number_of_first_list_to_be_submited' -log_dir log_files_directory -path 'path_to_currect_DijetRootTreeAnalyzer' -job_time 'tomorrow' "

	echo "Example: $0 -list_No 0 -log_dir Condor_Outputs/logs_JetHT__Run2018A-17Sep2018-v1__MINIAOD/ -path /afs/cern.ch/work/d/dkarasav/public/CMSSW_10_1_5/src/CMSDIJET/DijetRootTreeAnalyzer/ -job_time tomorrow"
...
do
  case "$1" in
	...
	-path) Full_path_to_DijetRootTreeAnalyzer="$2"; shift ;;  
...
#############################################################Creates the .sub file for condor submition#######################################
	# if a condor setup file with the same name exists in the log directory, it gets deleted and replaced.
	...
	echo "+JobFlavour 	          = \""$job_time"\"" 																	>> "$log_directory"/condor_setup_job_"$list_No".sub
	...
	echo "Arguments               = "$log_directory"/config_file_"$list_No".txt ""$Full_path_to_DijetRootTreeAnalyzer" " 0"  >> "$log_directory"/condor_setup_job_"$list_No".sub
