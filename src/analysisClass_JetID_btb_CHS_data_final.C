		...
		if(no_jets_AK4 >=2)
		 {
			...
			if(cut1 && cut2)
			{
				...
				{  
					...
					//	if (neMultAK4->at(0)>0)cout << neMultAK4->at(0) << endl;
						...
						eta_j1  = jetEtaAK4->at(0);		eta_j2  = jetEtaAK4->at(1);
						cm_j1   = chMultAK4->at(0);		cm_j2   = chMultAK4->at(1);
						nm_j1   = neMultAK4->at(0);		nm_j2   = neMultAK4->at(1);
						nemf_j1 = jetNemfAK4->at(0);	nemf_j2 = jetNemfAK4->at(1);
						cemf_j1 = jetCemfAK4->at(0);	cemf_j2 = jetCemfAK4->at(1);
						chf_j1  = jetChfAK4->at(0);		chf_j2  = jetChfAK4->at(1);
						nhf_j1  = jetNhfAK4->at(0);		nhf_j2  = jetNhfAK4->at(1);
						muf_j1  = jetMufAK4->at(0);		muf_j2  = jetMufAK4->at(1);
...
bool analysisClass::getJetID(int era, bool LepVeto, double eta, double cm, double nm, double nemf, double cemf, double nhf, double chf, double muf )
{
	...
	else if( era == 2017 )
	{
		...
		extracut_27_3 = (abs(eta)> 2.7 && abs(eta)<=3.0 && nemf>0.02 && nemf< 0.99 && nm>2	);  
		extracut_3_5 = (abs(eta)> 3.0 && abs(eta)<=5.0 && nemf< 0.9 && nhf>0.02 && nm>10); 
	...
	else 
	{
		...
		extracut_26_27 = (abs(eta)> 2.6 && abs(eta)<=2.7 && cm>0 && nemf<0.99 && nhf<0.9 );
//		extracut_27_3 = (abs(eta)> 2.7 && abs(eta)<=3.0 && nemf>0.02 && nemf < 0.99 && nm> 2);  
		extracut_27_3 = (abs(eta)> 2.7 && abs(eta)<=3.0 && nemf>0.01 && nemf < 0.99 && nm> 2);  //UL 17 criteria
		extracut_3_5 = (abs(eta)> 3.0 && abs(eta)<=5.0 && nemf< 0.9 && nhf>0.2 && nm>10 ); 
		lepton_cut =  abs(eta)>2.7 || ( muf<0.8 && cemf<0.8 && abs(eta)<=2.7 );
