...
#include "TH1.h"
	...
	void Stitch_significances(){

	double method_change_mass = 3000.0; // mass at which the two methods change
	...
	char *limits1_directory = "Standard_Fit_full_dataset_magdas_processing/Narrow_limits/significance/";       				 //directory with Standard Fit limits
	char *limits2_directory = "Simultaneous_fit_testing/Results_with_ax3/Narrow_resonances/significances/Narrow_qq_significance/";  		 //directory with Ratio Method limits	
	char *combined_directory ="Simultaneous_fit_testing/Results_with_ax3/Stiched_Results/Narrow_resonances/significances/";	
	char *resonance = "qq";          // give the resonance type 

	int year = 2017;
	char box1[300], box2[300];

		if      ( year == 2016){ strcpy(box1,"_PFDijet2016");
			    		 strcpy(box2,"_PFDijet2016MC");
				}
		else if ( year == 2017){ strcpy(box1,"_PFDijet2017");
	...
	strcat(output_directory,"xsecUL_ProfileLikelihood_");
...
//////creating strings with "/D" on end//////////////////////////
	...
	int flag=0;
		for (int i=0; i<nentries1; i++)    //event loop
		{
			if (i<=fabs(nentries1-nentries2)-1) {
					...
					//cout << " i = "<< i <<" massSF = "<<mass_SF << " xsec SF = "<< xsecUL3_SF <<" xsec Comb = "<< xsecUL3_Combined << endl;
			...
			if (i>fabs(nentries1-nentries2)-1)
			{
			      ...
			      tree1->GetEntry(i);
			      tree2->GetEntry(i-fabs(nentries1-nentries2));
					//cout << " i = "<< i <<" massSF = "<<mass_SF << endl;
					//cout << " i = "<< i <<" massRM = "<<mass_RM << endl;
				...
				else 
				{
					...
					//cout << " i = "<< i <<", mass = "<<mass_Combined  << ", xsec SF = "<< xsecUL3_SF<< ", xsec RM = "<< xsecUL3_RM <<", xsec Comb = "<< xsecUL3_Combined << endl;
