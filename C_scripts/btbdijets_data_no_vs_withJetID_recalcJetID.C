...
void btbdijets_data_no_vs_withJetID_recalcJetID()
 {

//	const char *input_directory="root://eoscms//eos/cms/store/group/phys_jetmet/dimitris/UL2017_btb_PUPPI_JetID_reduced/RunB/";	



	char analyzer_path[300] = "/afs/cern.ch/work/d/dkarasav/public/CMSSW_10_1_5/src/CMSDIJET/DijetRootTreeAnalyzer/"; 
	char output_directory[300] = "UL_datasets_JetID/UL_2016/PUPPI/APV/First_try/no_vs_withJetID";     
	char image_name[300] = "RunBver2_PFJet200"; 
...
//		h_ETAJet1_with[h] = new TH1D(name, "", 60,-4,4);
	...
	TH2D *h_NM_pT_map = new TH2D("h_NM_pT_map", "NM-pT map", 100 , 0, 100, 300 ,0.,3000.);
	TH2D *h_NM_pT_map_with = new TH2D("h_NM_pT_map_with", "NM-pT map", 100 , 0, 100, 300 ,0.,3000.);
...
//	tree->Add("/eos/cms/store/group/phys_jetmet/dimitris/UL2016_data_reduced_PUPPI/APV/JetHT__Run2016Bver1-21Feb2020_UL2016_MINIAOD_reduced_skim.root");
	tree->Add("/eos/cms/store/group/phys_jetmet/dimitris/UL2016_data_reduced_PUPPI/APV/JetHT__Run2016Bver2-21Feb2020_UL2016_MINIAOD_reduced_skim.root");
//	tree->Add("/eos/cms/store/group/phys_jetmet/dimitris/UL2016_data_reduced_PUPPI/APV/JetHT__Run2016C-21Feb2020_UL2016_MINIAOD_reduced_skim.root");
//	tree->Add("/eos/cms/store/group/phys_jetmet/dimitris/UL2016_data_reduced_PUPPI/APV/JetHT__Run2016D-21Feb2020_UL2016_MINIAOD_reduced_skim.root");
//	tree->Add("/eos/cms/store/group/phys_jetmet/dimitris/UL2016_data_reduced_PUPPI/APV/JetHT__Run2016E-21Feb2020_UL2016_MINIAOD_reduced_skim.root");
//	tree->Add("/eos/cms/store/group/phys_jetmet/dimitris/UL2016_data_reduced_PUPPI/APV/JetHT__Run2016F-21Feb2020_UL2016_MINIAOD_reduced_skim.root");

//	tree->Add("/eos/cms/store/group/phys_jetmet/dimitris/UL2016_data_reduced_PUPPI/nonAPV/JetHT__Run2016F-21Feb2020_UL2016_MINIAOD_reduced_skim.root");
//	tree->Add("/eos/cms/store/group/phys_jetmet/dimitris/UL2016_data_reduced_PUPPI/nonAPV/JetHT__Run2016G-21Feb2020_UL2016_MINIAOD_reduced_skim.root");
//	tree->Add("/eos/cms/store/group/phys_jetmet/dimitris/UL2016_data_reduced_PUPPI/nonAPV/JetHT__Run2016H-21Feb2020_UL2016_MINIAOD_reduced_skim.root");
...
//		if (passHLT4==1) h_ETAJet1->Fill(eta_j1);	
		if (passHLT2==1) h_ETAJet1->Fill(eta_j1);	
		if ( JetID_j1 && JetID_j2 && passHLT4==1 ) h_ETAJet1_with->Fill(eta_j1);
...
//		if ( passHLT2 != 1 ) continue; //trigger cut
		...
		{ 
			...
			if (ybin==6) h_NM_pT_map->Fill(nm_j1,pT_j1);
			h_eta_phi_map->Fill(eta_j1,phi_j1);
			}// end of if ybin>-1
		...
		//	h_ETAJet1_with[ybin]->Fill(eta_j1);
			...
			h_nVtx_with[ybin]->Fill(nVtx);
			if (ybin==6) h_NM_pT_map_with->Fill(nm_j1,pT_j1);
			h_eta_phi_map_with->Fill(eta_j1,phi_j1);
		}//end of if ybin>-1
	}// end of event loop
...
//	pad17->cd(iy+1);
	...
	//	leg1->Draw("same");
		...
		h_METovSUMET_with[iy]->Draw("same");
		paveCMS ->Draw("same");
		teta->Draw();
...
//		else pad18->cd()->SetLogy(1);
	...
	NM_pT_canvas->cd();
    NM_pT_canvas->cd()->SetRightMargin(0.18);
    NM_pT_canvas->cd()->SetTopMargin(0.07);
	h_NM_pT_map->SetStats(true);
	h_NM_pT_map->GetXaxis()->SetTitle("NM-pT of Jet1");
	h_NM_pT_map->GetXaxis()->SetTitle("Neutral Multiplicity") ;
	h_NM_pT_map->GetYaxis()->SetTitle("pT of Jet 1");
	h_NM_pT_map->GetYaxis()->SetTitleOffset(1.5);
	h_NM_pT_map->SetMinimum(1.);
	gPad->SetLogz();
	h_NM_pT_map->Draw("colz");   
	paveCMS->Draw("same");

	NM_pT_canvas_with->cd();
	NM_pT_canvas_with->cd()->SetRightMargin(0.18);
    NM_pT_canvas_with->cd()->SetTopMargin(0.07);
	h_NM_pT_map_with->SetStats(true);
	h_NM_pT_map_with->GetXaxis()->SetTitle("NM-pT of Jet1");
	h_NM_pT_map_with->GetXaxis()->SetTitle("Neutral Multiplicity") ;
	h_NM_pT_map_with->GetYaxis()->SetTitle("pT of Jet 1");
	h_NM_pT_map_with->GetYaxis()->SetTitleOffset(1.5);
	h_NM_pT_map_with->SetMinimum(1.);
	gPad->SetLogz();
	h_NM_pT_map_with->Draw("colz");   
	paveCMS->Draw("same");
	...
	{ 
		...
		sprintf(filename,"%s/%s/%s_%s_NM_pT.png",analyzer_path,output_directory,image_name,legend_1);
		NM_pT_canvas->SaveAs(filename);
		sprintf(filename,"%s/%s/%s_%s_NM_pT.png",analyzer_path,output_directory,image_name,legend_2);
		NM_pT_canvas_with->SaveAs(filename);
		/*
		sprintf(filename,"%s/%s/%s_%s_chf_jet1.pdf",analyzer_path,output_directory,image_name,c);
		pad1->SaveAs(filename);
		sprintf(filename,"%s/%s/%s_%s_nhf_jet1.pdf",analyzer_path,output_directory,image_name,c);
		pad2->SaveAs(filename);
		sprintf(filename,"%s/%s/%s_%s_cemf_jet1.pdf",analyzer_path,output_directory,image_name,c);
		pad3->SaveAs(filename);
		sprintf(filename,"%s/%s/%s_%s_nemf_jet1.pdf",analyzer_path,output_directory,image_name,c);
		pad4->SaveAs(filename);
		sprintf(filename,"%s/%s/%s_%s_muf_jet1.pdf",analyzer_path,output_directory,image_name,c);
		pad5->SaveAs(filename);
		sprintf(filename,"%s/%s/%s_%s_pt_jet1.pdf",analyzer_path,output_directory,image_name,c);
		pad9->SaveAs(filename);
		sprintf(filename,"%s/%s/%s_%s_metovsumet.pdf",analyzer_path,output_directory,image_name,c);
		pad10->SaveAs(filename);
		sprintf(filename,"%s/%s/%s_%s_NM.pdf",analyzer_path,output_directory,image_name,c);
		pad11->SaveAs(filename);
		sprintf(filename,"%s/%s/%s_%s_CM.pdf",analyzer_path,output_directory,image_name,c);
		pad12->SaveAs(filename);
		sprintf(filename,"%s/%s/%s_%s_PM.pdf",analyzer_path,output_directory,image_name,c);
		pad13->SaveAs(filename);
		sprintf(filename,"%s/%s/%s_%s_DijetMass.pdf",analyzer_path,output_directory,image_name,c);
		pad15->SaveAs(filename);
		sprintf(filename,"%s/%s/%s_%s_phi.pdf",analyzer_path,output_directory,image_name,c);
		pad16->SaveAs(filename);
		sprintf(filename,"%s/%s/%s_%s_eta.pdf",analyzer_path,output_directory,image_name,c);
		pad17->SaveAs(filename);
		*/
	//}
