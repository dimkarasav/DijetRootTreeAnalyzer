...
#                shapeString += '\t0.5'
        ...
        if penalty:
            slopeString = 'slope\tparam\t4.52\t1.49\n'
            betaString = 'beta\tparam\t0.988\t0.006\n'
            epsilonString = 'epsilon\tflatParam\n'
        else:
           # slopeString = "slope_%s\tflatParam\n"%(year)
            slopeString = 'slope\tflatParam\n'
            betaString = 'beta\tflatParam\n'
#            betaString = "beta_%s\tflatParam\n"%(year)
            epsilonString = 'epsilon\tflatParam\n'            
   ...
   #     if options.jesUpFile is not None or options.jesDownFile is not None:
    #        shapes.append('jes')
     #       shapeFiles['jesUp'] = options.jesUpFile
      #      shapeFiles['jesDown'] = options.jesDownFile
       ...
       #     shapeFiles['jesDown'] = options.jesDownFile
        if options.jesUpFile is not None or options.jesDownFile is not None:
            shapes.append('jes_%s'%year)
            shapeFiles['jes_%sUp'%year] = options.jesUpFile
            shapeFiles['jes_%sDown'%year] = options.jesDownFile
      #  if options.jerUpFile is not None or options.jerDownFile is not None:
       #     shapes.append('jer')
        #    shapeFiles['jerUp'] = options.jerUpFile
         #   shapeFiles['jerDown'] = options.jerDownFile
        if options.jerUpFile is not None or options.jerDownFile is not None:
            shapes.append('jer_%s'%year)
            shapeFiles['jer_%sUp'%year] = options.jerUpFile
            shapeFiles['jer_%sDown'%year] = options.jerDownFile
    ...
    if options.mcFile is not None and 'MC' in box and 'MCCR' not in box: #THIS IS USED FOR THE RATIO-PREDICTION
        ...
        if options.useRooParametricHist:
          ...
          #  w.factory('slope_%s[2.37,-20,20]'%(year)) # here is where I change slope # 5.14 for data # 1.0 for corrected ratio
#            w.factory('slope[2.37,-20,20]') # here is where I change slope # 5.14 for data # 1.0 for corrected ratio
            w.factory('slope[6.17,-20,20]') # for ax4 prediction
            w.factory('beta[0.984,0.5,1.5]') # for ax4 prediction
            #w.factory('beta_%s[0.973,0.5,1.5]'%(year)) # here is where I change beta # 0.976 for data # 1.018 for corrected ratio
#            w.factory('beta[0.973,0.5,1.5]') # here is where I change beta # 0.976 for data # 1.018 for corrected ratio
            w.factory('epsilon[1.]') # overall normalization term
            #w.factory('epsilon[1.,0.,100.]') # overall normalization term
            ...
            for iBinX in range(1,myRealTH1.GetNbinsX()+1):
                ...
                w.factory('mjjBin%i_%s[%f]'%(iBinX,year,mjjBin))
                #print mjjCen, mjbetajBin
                #w.factory('mcRatioBin%i[%f]'%(iBinX,myRealMCRatio.GetBinContent(iBinX)))
                w.factory('mcRatioBin%i_%s[%f]'%(iBinX,year,myMCRatio.GetBinContent(myMCRatio.FindBin(mjjBin))))
                w.factory('crBin%i_%s[%f,%f,%f]'%(iBinX,year,myRealCR.GetBinContent(iBinX),0,myRealCR.GetBinContent(iBinX)+myRealCR.GetBinError(iBinX)*50.))
                #w.factory("expr::bin%iFunc_%s('(@0+@1*(@2/@3)**3)*@4*@5*@6',beta,slope,mjjBin%i_%s,sqrts,mcRatioBin%i_%s,crBin%i_%s,epsilon)"%(iBinX,year,iBinX,year,iBinX,year,iBinX,year))

                w.factory("expr::bin%iFunc_%s('(@0+@1*(@2/@3)**4)*@4*@5*@6',beta,slope,mjjBin%i_%s,sqrts,mcRatioBin%i_%s,crBin%i_%s,epsilon)"%(iBinX,year,iBinX,year,iBinX,year,iBinX,year)) #ax4 prediction
                #w.factory("expr::bin%iFunc_%s('(@0+@1*(@2/@3)**3)*@4*@5*@6',beta_%s,slope_%s,mjjBin%i_%s,sqrts,mcRatioBin%i_%s,crBin%i_%s,epsilon)"%(iBinX,year,year,year,iBinX,year,iBinX,year,iBinX,year))

                binFunctions.add(w.function('bin%iFunc_%s'%(iBinX,year)))
                print iBinX, w.var('beta').getVal(), w.var('beta').getError(), w.var('slope').getVal(), w.var('mjjBin%i_%s'%(iBinX,year)).getVal(), myMCRatio.GetBinContent(myMCRatio.FindBin(mjjBin)), myRealCR.GetBinContent(iBinX), w.function('bin%iFunc_%s'%(iBinX,year)).getVal(), myRealTH1.GetBinContent(iBinX)
                #print iBinX, w.var('beta_%s'%(year)).getVal(), w.var('beta_%s'%(year)).getError(), w.var('slope_%s'%(year)).getVal(), w.var('mjjBin%i_%s'%(iBinX,year)).getVal(), myMCRatio.GetBinContent(myMCRatio.FindBin(mjjBin)), myRealCR.GetBinContent(iBinX), w.function('bin%iFunc_%s'%(iBinX,year)).getVal(), myRealTH1.GetBinContent(iBinX)
...
#######################################################################################################################
    if options.mcFile is not None and 'MCCRmid' in box: #THIS IS USED FOR THE RATIO-PREDICTION simultaneous fit SR, CR-middle,CR-high
       ...
       # myCR = mcFile.Get(cfg.getVariables(box+'CR', "histoName"))
        if options.mcFile is not None and 'PFDijet2016' in box:
            myCR = mcFile.Get(cfg.getVariables('PFDijet2016MCCR', "histoName"))
        elif options.mcFile is not None and 'PFDijet2017' in box:
            myCR = mcFile.Get(cfg.getVariables('PFDijet2017MCCR', "histoName"))   
        elif options.mcFile is not None and 'PFDijet2018' in box:
            myCR = mcFile.Get(cfg.getVariables('PFDijet2018MCCR', "histoName"))   
        myCR.Rebin(len(x)-1,'cr_rebin',x)        
        myRebinnedCR = rt.gDirectory.Get('cr_rebin')
        myRebinnedCR.SetDirectory(0)
        myRealCR = convertToTh1xHist(myRebinnedCR)
        ...
        if options.useRooParametricHist:
            binFunctions = rt.RooArgList()
       #     w.factory('slope[1.91,-20,20]') # here is where I change slope # 5.14 for data # 1.0 for corrected ratio
           # w.factory('slope_%s[1.91,-20,20]'%(year)) # here is where I change slope # 5.14 for data # 1.0 for corrected ratio
     #       w.factory('beta[0.979,0.5,1.5]') # here is where I change beta # 0.976 for data # 1.018 for corrected ratio
            w.factory('slope[6.17,-20,20]') # for ax4 prediction
            w.factory('beta[0.984,0.5,1.5]') # for ax4 prediction
#            w.factory('beta_%s[0.979,0.5,1.5]'%(year)) # here is where I change beta # 0.976 for data # 1.018 for corrected ratio
            w.factory('epsilon[1.]') # overall normalization term
            #w.factory('epsilon[1.,0.,100.]') # overall normalization term
            ...
            for iBinX in range(1,myRealTH1.GetNbinsX()+1):
                ...
                expB = 2.5E-03 # from fit of CR
                mjjBin = mjjLow + (1./expB) *(rt.TMath.Log(expB * mjjBinWidth) - rt.TMath.Log(1. - rt.TMath.Exp(-1. * expB * mjjBinWidth)))
                w.factory('mjjBin_mid%i_%s[%f]'%(iBinX,year,mjjBin))
                #print mjjCen, mjbetajBin
                #w.factory('mcRatioBin%i[%f]'%(iBinX,myRealMCRatio.GetBinContent(iBinX)))
                w.factory('mcRatioBin_mid%i_%s[%f]'%(iBinX,year,myMCRatio.GetBinContent(myMCRatio.FindBin(mjjBin))))
                w.factory('crBin%i_%s[%f,%f,%f]'%(iBinX,year,myRealCR.GetBinContent(iBinX),0,myRealCR.GetBinContent(iBinX)+myRealCR.GetBinError(iBinX)*50.))
             #   w.factory("expr::bin_mid%iFunc_%s('(@0+@1*(@2/@3)**3)*@4*@5*@6',beta,slope,mjjBin_mid%i_%s,sqrts,mcRatioBin_mid%i_%s,crBin%i_%s,epsilon)"%(iBinX,year,iBinX,year,iBinX,year,iBinX,year))
                w.factory("expr::bin_mid%iFunc_%s('(@0+@1*(@2/@3)**4)*@4*@5*@6',beta,slope,mjjBin_mid%i_%s,sqrts,mcRatioBin_mid%i_%s,crBin%i_%s,epsilon)"%(iBinX,year,iBinX,year,iBinX,year,iBinX,year)) #ax4 prediction
                #w.factory("expr::bin_mid%iFunc_%s('(@0+@1*(@2/@3)**3)*@4*@5*@6',beta_%s,slope_%s,mjjBin_mid%i_%s,sqrts,mcRatioBin_mid%i_%s,crBin%i_%s,epsilon)"%(iBinX,year,year,year,iBinX,year,iBinX,year,iBinX,year))

                binFunctions.add(w.function('bin_mid%iFunc_%s'%(iBinX,year)))
                print iBinX, w.var('beta').getVal(), w.var('beta').getError(), w.var('slope').getVal(), w.var('mjjBin_mid%i_%s'%(iBinX,year)).getVal(), myMCRatio.GetBinContent(myMCRatio.FindBin(mjjBin)), myRealCR.GetBinContent(iBinX), w.function('bin_mid%iFunc_%s'%(iBinX,year)).getVal(), myRealTH1.GetBinContent(iBinX)
               # print iBinX, w.var('beta_%s'%(year)).getVal(), w.var('beta_%s'%(year)).getError(), w.var('slope_%s'%(year)).getVal(), w.var('mjjBin_mid%i_%s'%(iBinX,year)).getVal(), myMCRatio.GetBinContent(myMCRatio.FindBin(mjjBin)), myRealCR.GetBinContent(iBinX), w.function('bin_mid%iFunc_%s'%(iBinX,year)).getVal(), myRealTH1.GetBinContent(iBinX)
            rph = rt.RooParametricHist('%s_bkg'%box,'%s_bkg'%box,th1x,binFunctions,myRealTH1)
            rph_norm = rt.RooAddition('%s_bkg_norm'%box,'%s_bkg_norm'%box,binFunctions)
            rootTools.Utils.importToWS(w,rph)
            rootTools.Utils.importToWS(w,rph_norm,rt.RooFit.RecycleConflictNodes())
#################################################################################################################
        ...
        if options.useRooParametricHist:
            ...
            for iBinX in range(1,myRealTH1.GetNbinsX()+1):
                w.factory('crBin%i_%s[%f,%f,%f]'%(iBinX,year,myRealTH1.GetBinContent(iBinX),0,myRealTH1.GetBinContent(iBinX)+myRealTH1.GetBinError(iBinX)*50.))
                binVariablesCR.add(w.var('crBin%i_%s'%(iBinX,year)))
