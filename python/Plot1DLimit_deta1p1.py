...
def getThyXsecDict():    
    thyXsecDict = {}
    xsecFiles = ['data/all_lowmass_lhc13TeV_1.txt','data/rsg_gg_lhc13TeV.txt','data/S8_13TeV_narrow_1.txt','data/string_total_13TeV_1.txt','data/axi_lhc13TeV_NLO_1.txt','data/dm_xsec_1_NLO_kFactor.txt','data/Zprimebb_xsec.txt','data/dmbb_xsec.txt']
    print xsecFiles
...
if __name__ == '__main__':
    ...
    parser.add_option('--combinedMethods',dest="combinedMethods",default=False,action='store_true',
                  help="for plotting combined method limits")
    ...
    if options.model=='gg':
        ...
        if 'PF' in options.box:
            thyModelsToDraw = ["W'","Z'",'DM1GeV','AxigluonNLO','E6Diquark']            
        elif 'PF' in options.box and 'bb' in options.box:
            thyModelsToDraw = ["Zprimebb",'DMbb1GeV']            
        else:
            thyModelsToDraw = ["W'","Z'",'DM1GeV','AxigluonkNLO','E6Diquark']            
    ...
    legendLabel = {'RSGravitonGG':'RS graviton (gg#rightarrowG#rightarrowgg)',
                   ...
                   'S8':'Color-octet scalar (k_{s}^{2} = #frac{1}{2}) ',
 ...
 #       h_limit.SetTitle(" ;Resonance mass [GeV]; #sigma#bf{#it{#Beta}}#bf{#it{#Alpha}} [pb]")
        h_limit.SetTitle(" ;Resonance mass [GeV]; #sigma#kern[0.15]{#bf{#it{#Beta}}}#kern[0.15]{#bf{#it{#Alpha}}} [pb]")
    ...
    for Box in Boxes:
        for model in models:    
            if options.doSignificance:
                ...
                for thyModel in thyModelsToDraw:
                    xsec_gr_nom[thyModel].Draw("c same")        # theory model change
                gr_observedLimit[(Box,model)].Draw("lp SAME")
  ...
  #  if len(Boxes)>1 and len(models)>1:
     #   l.DrawLatex(0.28,0.92,"CMS #it{Preliminary}")

#        l.DrawLatex(0.2,0.85,"CMS")
 #   elif len(Boxes)>1:
  #      l.DrawLatex(0.2,0.85,"CMS")

      #  l.DrawLatex(0.39,0.92,"CMS #it{Preliminary}")
#    else:

#        l.DrawLatex(0.2,0.85,"CMS")
       # l.DrawLatex(0.17,0.92,"CMS #it{Preliminary}")
   ...
   # l.DrawLatex(0.60,0.92,"%.1f fb^{-1} (13 TeV)"%(options.lumi))
    l.DrawLatex(0.63,0.92,"%.0f fb^{-1} (13 TeV)"%(options.lumi))
    
    if options.model=="gg":
        if len(Boxes)>1:
            l.DrawLatex(0.57,0.62,"gluon-gluon")
        else:
            l.DrawLatex(0.57,0.62,"gluon-gluon")
    elif options.model=="qg":        
        if len(Boxes)>1:
            l.DrawLatex(0.57,0.62,"quark-gluon")
        else:
            l.DrawLatex(0.57,0.62,"quark-gluon")
    elif options.model=="qq":
        if len(Boxes)>1:
            l.DrawLatex(0.57,0.62,"quark-quark")
        else:
            l.DrawLatex(0.57,0.62,"quark-quark")
...
#    elif options.model =="gg_qg_qq" and options.box=="CaloDijet2016_PFDijet2016":
        ...
        leg = rt.TLegend(0.19,0.17,0.47,0.32)        
    else:        
...
#            leg.AddEntry(gr_expectedLimit1sigma[(Box,model)], "#pm 1 s.t.d. dev.","lf")    
            leg.AddEntry(gr_expectedLimit1sigma[(Box,model)], "#pm 1 std. deviation","lf")    
        if not options.doSignificance:
#            leg.AddEntry(gr_expectedLimit2sigma[(Box,model)], "Expected #pm 2 s.d.","lf")
#            leg.AddEntry(gr_expectedLimit2sigma[(Box,model)], "#pm 2 s.t.d. dev.","lf")
            leg.AddEntry(gr_expectedLimit2sigma[(Box,model)], "#pm 2 std. deviation","lf")    
    else:
        #leg.AddEntry(None,"95% CL limits","")
        #leg.AddEntry(None,"90% CL limits","")
        for model in models:
    ...
    leg.Draw("SAME")   # model label here
...
#        if options.model =="gg_qg_qq" and options.box=="CaloDijet2016_PFDijet2016":
        if options.model =="gg_qg_qq":
            ...
            legThyModel = rt.TLegend(0.48,0.7,0.9,0.92)
            legThyModel2 = rt.TLegend(0.58,0.54,0.9,0.7)            
            ...
            legThyModel2.SetFillStyle(0)
        elif options.model =="gg":
            #legThyModel = rt.TLegend(0.2,0.17,0.6,0.4)
            legThyModel = rt.TLegend(0.2,0.17,0.63,0.4)
        elif options.model =="rsg":
            legThyModel = rt.TLegend(0.2,0.17,0.45,0.4)
        else:
            legThyModel = rt.TLegend(0.2,0.17,0.55,0.4)
        #if options.model =="rsg":
          #  legThyModel.SetTextSize(0.01)
	...
	legThyModel.SetFillStyle(0)
        ...
        legThyModel.Draw("same")     #model change here
        try:
	...
	h_limit.GetXaxis().SetLabelOffset(1000)
        ...
        if options.combinedMethods:
            if options.doSignificance:  
                line5 = rt.TLine(2950,0,2950,4)
            else:
                line5 = rt.TLine(2950,0.01,2950,options.xsecMax)
            ...
            l5.SetTextFont(72)
            if options.model =="gg_qg_qq":
                l5.DrawLatex(0.195,0.77,"Fit")
                l5.DrawLatex(0.195,0.73,"method")
               # l5.DrawLatex(0.185,0.641,"Method")
                l5.DrawLatex(0.32,0.77,"Ratio")
                l5.DrawLatex(0.32,0.73,"method")
            else: 
                l5.DrawLatex(0.195,0.80,"Fit")
                l5.DrawLatex(0.195,0.76,"method")
               # l5.DrawLatex(0.185,0.641,"Method")
                l5.DrawLatex(0.32,0.80,"Ratio")
                l5.DrawLatex(0.32,0.76,"method")
        xLabCMS = rt.TLatex()
        xLabCMS.SetTextAlign(22)
#        xLabCMS.SetTextAlign(11)
        xLabCMS.SetTextSize(0.045)
        xLabCMS.SetTextFont(62)
    #    xLabCMS.DrawLatex(2100, 30, "CMS")
        if not model=='gg_qg_qq':
            xLabCMS.DrawLatex(2500, 40, "CMS")
        else:
            xLabCMS.DrawLatex(2500, 400, "CMS")
        ...
        if options.doSignificance:
            ...
            yOffset = 5.25e-6 # for 1e-5 min
        for i in range(1,10):
            if i*1000>=options.massMin and i*1000<=options.massMax:
